...
(function () {
	...
	var openStart = getParameterByName('open_start');
	if (openStart) {
		var _openField = document.querySelector('select[name="open_start"]');
		for (var index = 0; index < _openField.options.length; index++) {
			if (_openField.options[index].value === openStart) {
				_openField.selectedIndex = index;
			}
		}
	}
  ...
  */
	...
	if ('geolocation' in navigator) {
		(function () {
			...
			var currentLocationButtons = document.getElementsByClassName('js-current-location');
			var addressField = form.querySelectorAll('input[name="address"]')[0];
				...
				button.addEventListener('click', function (e) {
					...
					getCurrentLocation().then(function (results) {
						if (results.length) {
							...
							addressField.value = results[1] ? results[1].formatted_address : results[0].formatted_address;
							//form.submit();
						}
